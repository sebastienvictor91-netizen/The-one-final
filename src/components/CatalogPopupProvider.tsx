  ...
  return (
    <CatalogCtx.Provider value={{ openPopup: () => setOpen(true), closePopup: () => setOpen(false) }}>
      ...
      {open && (
        ...
        >
          <div className="relative z-[10000] w-full max-w-lg rounded-2xl bg-card shadow-card border border-border overflow-hidden">
            <div className="h-1 fr-stripe" />

            <button
              onClick={() => setOpen(false)}
              aria-label="Fermer"
              className="absolute top-3 right-3 z-[10001] p-1.5 rounded-full hover:bg-secondary transition"
            >
              <X className="size-4" />
            </button>
